...
describe('AchievementsService', () => {
  ...
  it('should evaluate and award achievement idempotently', async () => {
    mockPrisma.achievement.findUnique.mockResolvedValue({ id: 'a1', code: 'first_task' });
    mockPrisma.userAchievement.findUnique.mockResolvedValue(null); // not awarded yet

    const result = await service.evaluateAchievement({ userId: 'u1', prismaTx: mockPrisma }, 'first_task');
  ...
  it('should check task count achievements', async () => {
    ...
    const evaluateSpy = jest.spyOn(service, 'evaluateAchievement').mockResolvedValue(true);

    await service.checkTaskCountAchievements({ userId: 'u1', prismaTx: mockPrisma });
  ...
  it('should check streak achievements', async () => {
    ...
    await service.checkStreakAchievements({ userId: 'u1', prismaTx: mockPrisma }, 7);
