...
export class RoadmapShAdapter implements RoadmapSourceAdapter {
  canHandle(input: string): boolean {
    return (
      input.includes('roadmap.sh') ||
      input.startsWith('roadmapsh:') ||
  ...
  async normalize(input: string): Promise<NormalizedRoadmap> {
    ...
    if (input.trim().startsWith('{') || input.trim().startsWith('[')) {
      try {
        ...
        throw new BadRequestException(
          'Invalid JSON provided to RoadmapShAdapter',
        );
      }
      ...
      try {
        ...
        if (!res.ok) {
          throw new BadRequestException(
            `Failed to fetch roadmap from ${input}: HTTP status ${res.status}`,
          );
        }
        ...
        throw new BadRequestException(
          `Could not fetch or parse roadmap from URL: ${input} (${e.message || e})`,
        );
      }
    } else {
      throw new BadRequestException(
        'RoadmapShAdapter requires a valid JSON payload or HTTP URL.',
      );
    }
    ...
    const nodes: NormalizedRoadmapNode[] = (rawData.nodes || []).map(
