...
import { MappingStatus, TaskPriority, TaskStatus, RoadmapSourceType, RoadmapNodeType, RoadmapStatus, RoadmapPriority } from '@prisma/client';
...
describe('RoadmapMaterializationService', () => {
  ...
  beforeEach(async () => {
    prismaService = {
      roadmap: {
        findUnique: jest.fn().mockImplementation(({ where }) => {
          ...
          if (where.id === 'rm-paused') return Promise.resolve({ ...mockRoadmap, id: 'rm-paused', status: RoadmapStatus.PAUSED });
      ...
      task: {
        ...
        findUnique: jest.fn().mockImplementation(({ where }) => Promise.resolve({ id: where.id, title: 'Task' })),
        create: jest.fn().mockImplementation(({ data }) => Promise.resolve({ id: 'task-gen', ...data })),
      },
      roadmapMapping: {
        update: jest.fn().mockImplementation(({ data }) => Promise.resolve({ id: 'map-updated', ...data })),
      },
    };
    ...
    tasksService = {
      createTask: jest.fn().mockImplementation((userId, milestoneId, data) =>
        Promise.resolve({ id: 'task-gen', journeyId: 'j-1', milestoneId, title: data.title, status: 'TODO' })
      ),
    };
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: RoadmapReconciliationService, useValue: reconciliationService },
        { provide: TasksService, useValue: tasksService },
      ],
    }).compile();

    service = module.get<RoadmapMaterializationService>(RoadmapMaterializationService);
  });
  ...
  it('2. Future nodes remain unmaterialized (bounded actionable window)', async () => {
    const tasks = await service.materializeActionableTasks('user-1', 'rm-1', { maxTasks: 1 });
    expect(tasks.length).toBe(1);
    expect(tasks[0].title).toBe('Python Basics');
  });
      ...
      nodes: [
        {
          ...
          mappings: [{ id: 'map-proj', taskId: null, mappingStatus: MappingStatus.COMPLETED, projectId: 'proj-1' }],
        },
      ],
    };
  ...
  it('15 & 16. Paused roadmap is excluded from Daily Focus and materialization', async () => {
    ...
    await expect(service.materializeActionableTasks('user-1', 'rm-paused')).rejects.toThrow(BadRequestException);
  ...
  it('8, 15, 16 & 17. Roadmap priority affects Daily Focus ranking (PRIMARY > SECONDARY)', async () => {
    const secondaryRoadmap = {
      ...
      snapshots: [{ ...mockSnapshot, id: 'snap-2', nodes: [{ id: 'node-sec', title: 'Network Security', sortOrder: 1, mappings: [] }] }],
    };
    prismaService.roadmap.findMany.mockResolvedValue([mockRoadmap, secondaryRoadmap]);
  ...
  it('19 & 20. Daily Focus produces explainable reasons with [Why?] grounding', async () => {
    const dailyFocus = await service.getDailyFocus('user-1', 2.5);
    expect(dailyFocus[0].whyReason).toContain('because this is the next actionable node');
  });
