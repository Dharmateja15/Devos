...
export function simulateMigration(legacySnapshots: LegacySnapshotFixture[]) {
  ...
  const unlinkedNoUrl = legacySnapshots.filter(
    (s) => !normalizeUrl(s.sourceUrl) && !snapshotLinks.has(s.id),
  );
...
describe('Roadmap Migration Integrity & Deterministic Grouping', () => {
  ...
  it('Case 2: Same user + same title + different URL -> assigned to SEPARATE Roadmaps', () => {
    const legacySnapshots: LegacySnapshotFixture[] = [
      ...
      },
      {
        id: 's2',
        userId: 'u1',
        sourceType: RoadmapSourceType.DOCUMENT_FALLBACK,
        sourceUrl: 'https://custom.example/ai',
        sourceName: 'AI Engineer',
        importedAt: new Date('2026-02-01'),
      },
    ];
    ...
    expect(migratedSnapshots[0].roadmapId).not.toBe(
      migratedSnapshots[1].roadmapId,
    );
  });
  ...
  it('Guarantees 0 orphaned snapshots and 100% relational integrity', () => {
    ...
    expect(
      migratedSnapshots.every((s) => s.roadmapId && s.roadmapId.length > 0),
    ).toBe(true);
