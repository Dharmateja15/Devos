...
describe('LearningService', () => {
  ...
  describe('Core Mechanics', () => {
    it('Completion != Mastery (without verified evidence it is ASSESSED at best)', async () => {
      ...
      jest
        .spyOn(prisma.learnerConceptState, 'findUnique')
        .mockResolvedValue({ state: LearnerState.UNKNOWN } as any);
      jest
        .spyOn(prisma.learnerConceptState, 'upsert')
        .mockResolvedValue({ state: LearnerState.ASSESSED } as any);
    ...
    it('Self-report produces SELF_REPORTED, not MASTERED', async () => {
      ...
      jest
        .spyOn(prisma.learnerConceptState, 'upsert')
        .mockResolvedValue({ state: LearnerState.SELF_REPORTED } as any);
    ...
    it('AI_ASSISTED does not invalidate work, grants ASSESSED without numerical penalty', async () => {
      jest.spyOn(prisma.task, 'findUnique').mockResolvedValue({
        id: 't1',
        status: TaskStatus.DONE,
        independenceSignal: IndependenceSignal.AI_ASSISTED,
        evidence: [{ verified: true, evidenceType: EvidenceType.MANUAL }],
      } as any);
      jest
        .spyOn(prisma.learnerConceptState, 'findUnique')
        .mockResolvedValue({ state: LearnerState.UNKNOWN } as any);
    ...
    it('Independent verified evidence strengthens state (Contextual Mastery)', async () => {
      ...
      jest
        .spyOn(prisma.learnerConceptState, 'upsert')
        .mockResolvedValue({ state: LearnerState.MASTERED } as any);
      const res = await service.evaluateTaskEvidence('u1', 'c1', 't1');
    ...
    it('Skip/Defer/Schedule preserves history and state', async () => {
      jest
        .spyOn(prisma.learnerConceptState, 'findUnique')
        .mockResolvedValue({ state: LearnerState.ASSESSED } as any);
      jest.spyOn(prisma.learnerConceptState, 'upsert').mockResolvedValue({
        state: LearnerState.ASSESSED,
        userIntent: 'DEFER',
        nextReviewAt: new Date(2050, 1, 1),
      } as any);
      const res = await service.setUserIntent(
        'u1',
        'c1',
        'DEFER',
        new Date(2050, 1, 1),
  ...
  describe('Mastery Check Flow', () => {
    it('Traditional Quiz is mandatory first mastery mode', async () => {
      ...
      await expect(
        service.submitMasteryCheck('u1', 'c1', {
          mode: 'EXPLANATION',
          passed: true,
        }),
    ...
    it('Optional mastery modes work', async () => {
      ...
      jest
        .spyOn(prisma.learnerConceptState, 'upsert')
        .mockResolvedValue({ state: LearnerState.MASTERED } as any);
      const res = await service.submitMasteryCheck('u1', 'c1', {
        mode: 'EXPLANATION',
        passed: true,
      });
      expect(res.state).toBe(LearnerState.MASTERED);
    ...
    it('Failed mastery check does not lock roadmap', async () => {
      ...
      jest
        .spyOn(prisma.learnerConceptState, 'upsert')
        .mockResolvedValue({ state: LearnerState.NEEDS_REVIEW } as any);
      const res = await service.submitMasteryCheck('u1', 'c1', {
        mode: 'TRADITIONAL_QUIZ',
        passed: false,
      });
