...
export interface MasteryCheckOptions {
  mode:
    'TRADITIONAL_QUIZ' | 'EXPLANATION' | 'DEBUGGING' | 'SCENARIO' | 'TRANSFER';
  passed: boolean;
...
export class LearningService {
  ...
  async selfReportConcept(userId: string, conceptId: string) {
    ...
    if (
      'state' in current &&
      (current.state === LearnerState.MASTERED ||
        current.state === LearnerState.ASSESSED)
    ) {
      return current;
    }
  ...
  async evaluateTaskEvidence(
    ...
    if (task.status !== TaskStatus.DONE) {
      throw new BadRequestException(
        'Task must be completed to evaluate mastery',
      );
    }

    const current = await this.getLearnerState(userId, conceptId);
    const currentStateStr =
      'state' in current ? current.state : LearnerState.UNKNOWN;

    // AI_ASSISTED is legitimate work, not numerically punished.
    let newState: LearnerState = currentStateStr;
    if (
      newState === LearnerState.UNKNOWN ||
      newState === LearnerState.SELF_REPORTED
    ) {
    ...
    const hasHighQualityEvidence = task.evidence.some(
      (e) =>
        e.evidenceType === EvidenceType.GITHUB_PR ||
        e.evidenceType === EvidenceType.GITHUB_COMMIT,
    );
    ...
    if (
      currentStateStr === LearnerState.ASSESSED &&
      hasVerifiedEvidence &&
      (isIndependent || hasHighQualityEvidence)
    ) {
  ...
  async submitMasteryCheck(
    ...
    if (masteryTasks === 0 && mode !== 'TRADITIONAL_QUIZ') {
      throw new BadRequestException(
        'TRADITIONAL_QUIZ is mandatory as the first mastery-check mode.',
      );
    }
  ...
  async setUserIntent(
    ...
    const current = await this.getLearnerState(userId, conceptId);
    const state = 'state' in current ? current.state : LearnerState.UNKNOWN;
