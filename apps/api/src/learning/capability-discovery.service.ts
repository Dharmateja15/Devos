...
export class CapabilityDiscoveryService {
   ...
   */
  async getDiscoveredCapabilities(userId: string): Promise<CapabilityDiscoveryResponseDto> {
    ...
    for (const ev of evidenceItems) {
      ...
      for (const tech of detectedTech) {
        ...
        if (ev.verified && (verificationStatus === 'REPOSITORY_OWNER_VERIFIED' || verificationStatus === 'CONNECTED_ACCOUNT_ASSOCIATION')) {
          group.signals.add('VERIFIED_GITHUB_REPOSITORY');
          group.strengths.push('MEDIUM');
        } else if (ev.verified && verificationStatus === 'COMMIT_AUTHOR_MATCH') {
    ...
    for (const [_, data] of capabilityMap.entries()) {
      ...
      // Sort timestamps
      const validDates = data.timestamps.filter(d => !isNaN(d.getTime())).sort((a, b) => a.getTime() - b.getTime());
      const firstObservedAt = validDates[0];
      const lastObservedAt = validDates[validDates.length - 1];

      const isCanonical = Boolean(data.canonicalConceptId || data.canonicalSkillId);
      ...
      capabilities.push({
        capabilityId: data.canonicalConceptId || data.canonicalSkillId || `tech-${data.title.toLowerCase().replace(/[^a-z0-9]/g, '-')}`,
