...
export class RecommendationService {
  ...
  async getRecommendations(userId: string): Promise<Recommendation[]> {
    ...
    for (const concept of concepts) {
      ...
      const isStale = this.learningService.isStale(
        stateRecord?.lastEvaluatedAt || null,
      );
      ...
      if (state === LearnerState.UNKNOWN) {
        ...
        recommendations.push({
          ...
          title: concept.title,
          type: 'PRACTICE',
          reason: 'Practice / Verification needed',
          isBlocked: false,
        });
      } else if (
        state === LearnerState.NEEDS_REVIEW ||
        (state === LearnerState.MASTERED && isStale)
      ) {
        recommendations.push({
          conceptId: concept.id,
          title: concept.title,
          type: 'REVIEW',
          reason: 'Review / Recall / Reinforcement',
          isBlocked: false,
        });
        ...
        recommendations.push({
          ...
          title: concept.title,
          type: 'PROGRESSION',
          reason: 'Mastered and Fresh. Ready for next step.',
          isBlocked: false,
        });
      }
    }
