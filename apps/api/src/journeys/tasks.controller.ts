...
export class TasksController {
  ...
  create(@CurrentUser() user: any, @Param('milestoneId') milestoneId: string, @Body() body: any) {
  ...
  complete(@CurrentUser() user: any, @Param('id') id: string, @Body() body: any) {
    return this.tasksService.completeTask(user.id, id, body?.independenceSignal);
  }
