...
export function NodeDetailPanel({
  ...
  const discovered = mappedIntelligence?.discoveredCapability;
  ...
  return (
    <div className="fixed inset-y-0 right-0 z-40 w-full max-w-md bg-white border-l border-slate-200 shadow-2xl flex flex-col transform transition-transform duration-300">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        {(freshness?.learnerState || discovered?.learnerState) && (
          <div>
        ...
        {/* 5B Extension Slot 1: Knowledge Freshness & Recency */}
          ...
          <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-3">
            ...
            {freshnessRecommendation && (
              <div className="pt-2 border-t border-slate-200/60 text-xs">
                ...
                <span className="text-indigo-600 font-bold">{freshnessRecommendation.recommendationType}</span>
                <p className="text-slate-500 mt-0.5">{freshnessRecommendation.whyReason}</p>
              </div>
            )}
          </div>
        )}

        {/* 5B Extension Slot 2: Conflict Analysis */}
        {conflict && (
        ...
        {/* 5C Project Evidence Submission Slot */}
        <div className="p-4 bg-slate-50 rounded-xl border border-slate-200 space-y-3">
          <div className="flex items-center justify-between">
          ...
          {/* 5C GitHub Evidence Submission Trigger */}
          <button
            onClick={() => setIsEvidenceModalOpen(true)}
            className="w-full mt-2 inline-flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-medium px-4 py-2 rounded-lg text-xs transition-colors shadow-sm"
          >
        ...
        {/* Discovered Capabilities & Evidence */}
        {discovered && (
          <div className="p-4 bg-purple-50/60 rounded-xl border border-purple-100 space-y-2">
            <div className="flex items-center justify-between">
              <h4 className="text-xs font-bold text-purple-950 uppercase tracking-wider">Discovered Capability</h4>
              <span className="text-xs font-bold px-2 py-0.5 rounded bg-purple-200 text-purple-800">
                {discovered.signalStrength} SIGNAL
              </span>
            </div>
            {discovered.evidenceSources.length > 0 && (
              <div className="text-xs text-purple-900">
                <strong>Evidence Sources:</strong> {discovered.evidenceSources.join(', ')}
              </div>
            )}
          </div>
        )}
        ...
        {mapping && !isKnown && (
          <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-3">
            ...
            <button
              ...
              className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-medium px-4 py-2 rounded-lg text-sm transition-colors disabled:opacity-50"
            >
          ...
          {parentNode && (
            <div>
              ...
              <button
                ...
                className="w-full text-left p-3 rounded-xl border border-slate-200 hover:border-indigo-400 bg-white hover:bg-indigo-50/50 transition-colors text-sm font-medium text-slate-800 flex items-center justify-between"
              >
              ...
              {node.resourceUrls.map((url, index) => (
                <li key={index}>
                  <a
                    ...
                    className="text-sm text-indigo-600 hover:text-indigo-800 underline truncate block flex items-center gap-1.5"
                  >
      ...
      {/* Phase 5C GitHub Evidence Submission Modal */}
